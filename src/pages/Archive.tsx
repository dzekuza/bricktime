...
function dbToProduct(row: Record<string, unknown>): Product {
  ...
  if (row.is_new) { badge = 'new'; badgeLabel = 'Nauja' }
  else if (status === 'sold_out') { badge = 'sold-out'; badgeLabel = 'Išnuomota' }
  else if (status === 'limited') { badge = 'limited'; badgeLabel = 'Ribotas' }
  return {
    id:           row.id as number,
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-3 p-4 md:p-5">
        <div>
          <h3 className="heading-display text-d-xs text-ink" style={{ lineHeight: '.95' }}>
            {product.title}
          </h3>
          <p className="mt-1 font-mono text-[11px] tracking-[.12em] uppercase text-ink/50">
            {product.subtitle}
          </p>
        </div>

        <p className="text-[13px] leading-[1.5] text-ink/60 line-clamp-2">
          {featured
          ...
          <div className="grid grid-cols-1 gap-7 md:grid-cols-2 lg:grid-cols-3">
            {loading
              ? Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="brick-card overflow-hidden animate-pulse">
                    ...
                    <div className="p-4 md:p-5 flex flex-col gap-3">
                      ...
                      <div className="h-9 rounded-xl bg-ink/10 mt-auto" />
                    </div>
                  </div>
                ))
