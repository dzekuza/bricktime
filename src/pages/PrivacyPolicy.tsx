...
const sections: Section[] = [
  ...
  },
  {
    ...
    bullets: [
      'Būtinieji slapukai naudojami svetainės veikimui, saugumui ir bazinėms funkcijoms užtikrinti.',
      'Analitiniams tikslams gali būti naudojami tokie slapukai kaip `_ga`, `ga*`, `_fbp` ir `_fbc`.',
        ...
        <section className="pb-8">
          <div className="mx-auto max-w-[1320px] px-4 md:px-7">
            <div className="grid gap-8 lg:grid-cols-[1.1fr_.9fr]">
              <div>
                <p className="label-mono text-ink/40">Teisinė informacija / Privatumas</p>
                ...
                <p className="mt-6 max-w-[62ch] text-[17px] leading-[1.7] text-ink/70">
                  Šiame puslapyje pateikiama informacija apie tai, kokius asmens duomenis tvarko
                  BRICKTIME, kokiais tikslais jie naudojami, kiek laiko saugomi ir kaip galite
              ...
              <aside className="brick-card self-start bg-ink p-6 text-paper md:p-8">
                ...
                <div className="mt-5 space-y-4">
                  {contactRows.map(([label, value]) => (
                    <div key={label} className="flex flex-col gap-1 border-b border-paper/12 pb-4 last:border-b-0 last:pb-0">
                      <span className="text-[12px] font-mono uppercase tracking-[0.08em] text-paper/45">
                        {label}
                      </span>
                      {label === 'El. paštas' ? (
                        <a href={`mailto:${value}`} className="text-[15px] leading-6 text-paper">
                          {value}
                        </a>
                      ) : (
                        <span className="text-[15px] leading-6 text-paper">{value}</span>
                      )}
                    </div>
                  ))}
                </div>
        ...
        <section className="pb-16 md:pb-24">
          <div className="mx-auto grid max-w-[1320px] gap-5 px-4 md:px-7">
            {sections.map((section, index) => (
              <article key={section.id} className="brick-card bg-paper p-6 md:p-8">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div>
                    <p className="label-mono text-ink/40">{String(index + 1).padStart(2, '0')}</p>
                    <h2 className="heading-display text-d-xs mt-3 text-ink">{section.title}</h2>
                  </div>
                  <a
                    href={`#${section.id}`}
                    className="font-mono text-[12px] uppercase tracking-[0.08em] text-ink/35"
                  >
                    #{section.id}
                  </a>
                </div>
                  ...
                  {section.bullets && (
                    <ul className="grid gap-3 pt-2">
                      {section.bullets.map((bullet) => (
                        <li key={bullet} className="flex gap-3 text-[15px] leading-7 text-ink/74 md:text-[16px]">
            ...
            <article className="brick-card bg-[#FFD731] p-6 md:p-8">
              ...
              <h2 className="heading-display text-d-xs mt-3 text-ink">Jei turite klausimų</h2>
              <p className="mt-4 max-w-[64ch] text-[15px] leading-7 text-ink/78 md:text-[16px]">
                Jei turėtumėte klausimų dėl šios privatumo politikos ar savo asmens duomenų
                tvarkymo, susisiekite el. paštu{' '}
                <a href="mailto:info@bricktime.lt" className="font-bold text-ink underline underline-offset-4">
                  info@bricktime.lt
                </a>{' '}
                arba telefonu{' '}
                <a href="tel:+37068211695" className="font-bold text-ink underline underline-offset-4">
                  +370 682 11695
                </a>
                .
              </p>
