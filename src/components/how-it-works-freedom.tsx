    ...
    <div className="reveal lg:col-span-12">
      <div className="brick-card bg-ink p-7 md:p-10">
        ...
        <p className="mt-5 max-w-[56ch] text-[15px] leading-[1.65] text-paper/70">
          Jokių ilgalaikių įsipareigojimų, nemokamas pristatymas ir prenumerata, prisitaikanti prie tavo konstravimo tempo. Kiekvieną mėnesį atrask naujus LEGO® konstravimo projektus.
        </p>
        <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {FREEDOM_FEATURES.map((f) => (
            <div key={f.num} className="rounded-2xl border-2 border-paper/15 p-5">
              <span className="label-mono text-brand-yellow">{f.num}</span>
              <h4 className="heading-display text-d-xs mt-2 text-paper">{f.title}</h4>
              <p className="mt-2 text-[13px] leading-[1.6] text-paper/60">{f.body}</p>
            </div>
          ))}
        </div>
      </div>
