---
name: Codex-md
description: Keep AGENTS.md current and lean — capture this session's durable learnings into it (default), or `audit` it for stale commands, drift, and bloat. Enforces the line budget either way.
argument-hint: "[audit?]"
disable-model-invocation: true
---

`AGENTS.md` loads every turn for every developer; this skill is how it earns that. Two modes: default captures learnings from the current session; `audit` checks the whole file against reality.
...
- **It's not derivable from the code.** Codex can re-discover file structure and signatures; it can't re-discover "the staging DB is the one named `prod_replica`".
- **It's durable.** True next month, not just this task. Session-specific state belongs in `/catchup handoff`, not here.
- **It's not already covered** by AGENTS.md, a rule in `.Codex/rules/`, or enforced by a hook.
...
2. Pick its destination: command fixes → Commands section; constraints with a path scope → suggest a path-scoped rule instead; everything else → the matching AGENTS.md section.
...
Check every line of AGENTS.md against reality:
...
3. **Duplication**: lines that repeat `.Codex/rules/` content, hook-enforced behavior, or standard conventions Codex already knows — propose deletion.
...
- Don't touch `Codex.local.md` — that's the user's personal space.
