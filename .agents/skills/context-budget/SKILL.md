---
name: context-budget
description: Estimate per-turn token cost of `.Codex/` and `AGENTS.md` — always-loaded vs path-scoped vs invoked-only, top contributors, over-budget flags. Add `--api` for exact tokenizer counts.
argument-hint: "[--api]"
disable-model-invocation: true
---

Estimate the token cost of this project's `.Codex/` configuration and `AGENTS.md` so the user can see exactly which files load every turn versus only when triggered.
...
find .Codex/rules -name '*.md' -type f 2>/dev/null
find .Codex/skills -name 'SKILL.md' -type f 2>/dev/null
find .Codex/agents -name '*.md' -type f 2>/dev/null
[ -f .Codex/AGENTS.md ] && echo "WARN: .Codex/AGENTS.md exists. AGENTS.md belongs at the project root."
```

Skip any file under `.Codex/agents/README.md`, `.Codex/rules/README.md`, etc. Those are folder descriptions, not loaded by Codex at runtime.

## Step 2: Classify rules by frontmatter

For every `.md` file in `.Codex/rules/`, read the YAML frontmatter (the block between the first two `---` lines).
...
Other categories:
- `./AGENTS.md` -> always-loaded (by definition).
- `.Codex/skills/<name>/SKILL.md` -> invoked-only (zero per-turn cost; only loads when the user runs `/skill-name` or, if `disable-model-invocation` is unset, when Codex auto-triggers it).
- `.Codex/agents/<name>.md` -> invoked-only and runs in isolated context (per-invocation cost in its own session, not per-turn cost in the main thread).
...
if [ -z "$ANTHROPIC_API_KEY" ]; then
  ...
  tokens=$(curl -s https://api.anthropic.com/v1/messages/count_tokens \
    ...
    -d "{\"model\":\"Codex-sonnet-4-6\",\"messages\":[{\"role\":\"user\",\"content\":$CONTENT}]}" \
    | jq -r '.input_tokens')
fi
```

The endpoint returns Anthropic's exact tokenizer count. That is the factual number Codex itself would see when loading the file.
...
Always-loaded (every turn):                 ~N tokens
  AGENTS.md                                  ~N tokens
  .Codex/rules/code-quality.md  (always)      ~N tokens
  .Codex/rules/testing.md       (always)      ~N tokens

Path-scoped (loaded near matched files):    ~N tokens (max, if every glob matches)
  .Codex/rules/security.md      (paths: src/api/**, ...)  ~N tokens
  .Codex/rules/frontend.md      (paths: **/*.tsx, ...)    ~N tokens

Invoked-only (zero per-turn cost):
  Cumulative size if every skill and agent loaded once: ~N tokens
  .Codex/skills/<name>/SKILL.md   ~N tokens
  .Codex/agents/<name>.md         ~N tokens
...
|-------------------------------|--------------------|--------------------|------------------|
| `AGENTS.md`                   | <25 non-blank lines | <50 non-blank lines | Trim per Phase 4 of `/setupdotclaude`. |
...
- Codex does not expose live context window state to skills, so this report estimates "what would load each turn for this configuration", not "what's currently in your session window".
